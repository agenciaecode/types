...
namespace Mkioschi\ValueObjects\PhoneNumber\Standards;

use Mkioschi\ValueObjects\PhoneNumber\PhoneNumberStandard;
...
class Br implements PhoneNumberStandard
{
    ...
    public function isValid(string $countryCode, string $areaCode, string $localNumber): bool
    {
        ...
        if (!in_array($areaCode, self::AREA_CODES)) return false;
        if (!is_numeric($localNumber) || !in_array(strlen($localNumber), [8, 9])) return false;
